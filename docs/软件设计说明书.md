...
| 5 年数据与 100ms 刷新目标 | 部分完成：5 年数据未导入；快照读取路径已实现，当前快照读取性能通过 100ms 检查 |
| 盘中实时监控 | 按用户决策不实现 |
| 当前测试 | `python backend/test_scoring.py` 通过；快照性能、数据覆盖检查、回测落库冒烟通过 |
...
| `em_daily_quote` | 个股日线 OCHL、成交量、成交额 | 已实现 | 105818 | 符合，但数据窗口尚非 5 年 |
| `em_sector` | 东方财富板块定义 | 已实现 | 1012 | 符合 |
| `em_sector_constituent_history` | 板块成分历史 | 已实现 | 87198 | 基本符合，仍需长期验证历史版本口径 |
| `local_sector_snapshot_daily` | 板块日度快照 | 已实现 | 220 | 部分符合，需扩展到 5 年 |
| `local_limit_signal_daily` | 涨停、触板、炸板、连板信号 | 已实现 | 100760 | 部分符合，当前为日线近似 |
...
| `local_audit_log` | 审计日志 | 已实现 | 783 | 符合本地审计 |
...
| `local_theme_ranking_snapshot` | 主线榜单预计算快照 | 已实现 | 1 | 默认 API 可命中快照 |
| `local_theme_matrix_snapshot` | 近 20 日矩阵预计算快照 | 已实现 | 1 | 当前快照读取约 12ms |
| `local_theme_detail_snapshot` | 主线详情预计算快照 | 已实现 | 100 | 当前为前 100 条主线详情 |
| `local_confidence_history_snapshot` | 置信度历史预计算快照 | 已实现 | 1 | 当前快照读取约 2ms |
| `local_risk_history_snapshot` | 风险历史预计算快照 | 已实现 | 100 | 当前为前 100 条主线风险历史 |
| `local_factor_effectiveness_snapshot` | 因子有效性预计算快照 | 已实现 | 1 | 当前保存 3 日持有期结果 |
| `local_snapshot_build_log` | 快照构建日志 | 已实现 | 12 | 记录构建耗时、状态和失败原因 |
...
| 近 20 日矩阵预计算 | `local_theme_matrix_snapshot`、`build_review_snapshots.py` | 已实现 | 当前构建约 36 秒，读取约 12ms |
...
| 快照读取 100ms 性能测试 | `backend/benchmark_snapshots.py` | 已实现 | 当前榜单约 4ms、矩阵约 12ms、详情/历史约 2ms |
| 5 年数据覆盖检查 | `backend/data_validation.py`、`/api/v1/data/coverage` | 已实现 | 当前输出 warning：只有 2026-04-01 至 2026-04-30 |
| 无未来函数风险检查 | `backend/data_validation.py`、`/api/v1/data/no-future-guard` | 部分实现 | 能识别板块成分缺历史版本风险 |
| 5 年性能测试 | 无 | 未实现 | 需在导入 5 年数据后复测 |
...
| 编号 | 问题 | 影响 | 后续动作 |
| --- | --- | --- | --- |
| G-001 | 尚未导入 5 年历史数据，覆盖检查已可见 | 回测、因子有效性、动态权重可信度不足 | 扩展导入日期范围，验证数据量和性能 |
...
| G-010 | 回测已持久化但非异步任务 | 长周期回测仍会阻塞请求 | 增加后台执行和状态轮询 |
