...
| 盘中实时监控 | 按用户决策不实现 |
| 当前测试 | `python backend/test_scoring.py` 通过；`python backend/benchmark_snapshots.py --date 2026-04-29 --days 20 --threshold-ms 100` 通过 |
...
| `local_backtest_run` | 回测任务持久化 | 未实现 | 0 | 待实现 |
...
| 编号 | 问题 | 影响 | 后续动作 |
| --- | --- | --- | --- |
| G-001 | 尚未导入 5 年历史数据 | 回测、因子有效性、动态权重可信度不足 | 扩展导入日期范围，验证数据量和性能 |
...
| G-010 | 回测非异步任务 | 长周期回测会阻塞请求 | 增加任务表和后台执行 |
