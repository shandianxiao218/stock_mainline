...
| 评分模型是否按第 8 章实现 | 基本实现；催化已按等级衰减入模，舆情为 AKShare 热度排行 + 行情代理融合 |
...
| G-002 | 页面 API 快照路径已在 5 年数据量下达标，但失效重建机制不足 | 参数、行情、催化、舆情变化后可能读到旧快照 | 建立快照依赖图、失效队列和重建任务 |
| G-003 | 完整真实舆情未接入 | 当前只是 AKShare 东财热度排行 + 行情代理增强 | 接入真实讨论量、搜索、新闻提及，并显示来源可信度 |
| G-004 | 催化事件已等级衰减入模，但自动分类不足 | 人工录入质量影响评分稳定性 | 增加新闻自动分类、来源可信度和人工修正审计 |
| G-005 | 自动聚合结果已版本化落库，但回测绑定历史版本不足 | 历史榜单复现仍可能受当前聚合逻辑影响 | 回测按交易日读取 `local_auto_theme_cluster` 对应版本 |
| G-006 | 涨停/炸板为日线近似 | 短线情绪和炸板率精度有限 | 接入更细粒度触板或盘口数据 |
| G-007 | 龙虎榜已接入，游资席位细分不足 | 游资参与字段仍偏粗 | 增加席位标签、席位类型和历史活跃度识别 |
...
| G-011 | 前端自动化测试已有基础覆盖，但未接入 CI | UI 回归仍依赖人工触发测试 | 扩展关键路径并接入自动化执行 |
