...
| 5 年数据与 100ms 刷新目标 | 未完成 |
| 盘中实时监控 | 按用户决策不实现 |
| 当前测试 | `python backend/test_scoring.py` 通过，17 个测试 OK |
...
| `local_theme_matrix_snapshot` | 近 20 日矩阵预计算快照 | 未实现 | 0 | 不符合 100ms 目标，需补 |
| `local_theme_detail_snapshot` | 主线详情预计算快照 | 未实现 | 0 | 不符合 100ms 目标，需补 |
| `local_factor_effectiveness_snapshot` | 因子有效性预计算快照 | 未实现 | 0 | 待实现 |
...
| 评分缓存 | `build_themes_for_date` 缓存相关逻辑 | 部分实现 | 仍有冷启动慢问题 |
| 100ms 首屏目标 | 未达到 | 未实现 | 需要快照表、索引、性能测试 |
| 近 20 日矩阵预计算 | 无 | 未实现 | `local_theme_matrix_snapshot` 待建 |
| 详情首屏预计算 | 无 | 未实现 | `local_theme_detail_snapshot` 待建 |
| 因子有效性预计算 | 无 | 未实现 | `local_factor_effectiveness_snapshot` 待建 |
| 异步回测任务 | 无 | 未实现 | 当前同步返回 |
...
| G-002 | 页面 API 仍有请求时重算 | 无法稳定满足 100ms 目标 | 建 `local_theme_matrix_snapshot`、`local_theme_detail_snapshot` 等快照 |
