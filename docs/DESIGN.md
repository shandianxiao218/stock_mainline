...
导入 5 年东方财富日线后，Web 页面刷新目标为 100ms 内完成首屏核心数据返回。该目标不能依赖请求时逐日重算，必须采用以下策略：

- 数据导入或收盘任务阶段预计算主线榜单、主线详情摘要、近 20 日矩阵、置信度历史、风险历史和常用成分股指标。
- 页面刷新 API 优先读取 `local_theme_score_daily`、`local_risk_signal_daily`、`local_confidence_daily` 和后续新增的本地快照表；请求时只做轻量筛选、排序和 JSON 组装。
- 评分引擎保留按需重算能力，用于模型调试、参数实验和回测任务，不作为页面刷新默认路径。
- SQLite 必须为 `trade_date`、`theme_id`、`sector_code`、`symbol`、`rank` 等查询路径建立索引；5 年数据导入后增加 100ms 响应时间回归测试。
- 回测任务继续走异步或批处理思路，不纳入 100ms 页面刷新目标。
...
- 主线榜单：无人工映射时从东方财富真实板块动态候选中评分输出；`GET /api/v1/themes/ranking` 默认返回前 10 个板块，`limit` 支持 `100` 和 `all`；导入 5 年数据后必须切换为优先读取预计算快照，页面刷新目标 100ms 内。
...
| 2026-05-01 | 个人版不实现盘中实时监测；导入 5 年数据后页面刷新目标为 100ms 内 | SRS Phase 4 包含盘中版本，性能需求原为首页 3 秒、榜单 2 秒 | 用户明确要求实时监测不用实现，并提出 5 年数据后的页面刷新 100ms 目标；因此必须走预计算和快照读取，而不是请求时逐日重算 | 范围裁剪、数据导入、评分任务、API 缓存、SQLite 索引、性能测试 |
